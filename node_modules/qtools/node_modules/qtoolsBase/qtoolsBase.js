...
var moduleFunction = function(self) {
	...
	self.dumpFlat = function(inData, returnStringFlag) {
	...
	self.extractDottedPaths = function(inObj) {
		var result = self.extractDottedPathRecursionEngine(inObj);
		delete self.pathList;
		return result;
	}

	self.extractDottedPathRecursionEngine = function(inObj, incomingPathString) {
		if (!self.pathList) {
			self.pathList = [];
		}
		...
		if (self.toType(inObj) != 'array') {
			...
			for (var i in inObj) {
				var suffix = '.' + i;
				if (inObj[i] !== null && typeof (inObj[i]) == "object") {
					self.extractDottedPathRecursionEngine(inObj[i], incomingPathString + suffix);
				} else {
					self.pathList.push((incomingPathString + suffix).replace(/\.$/, '').replace(/^\./, '')); //we accumulate leading and trailing dots, remove them
			...
			for (var i = 0, len = inObj.length; i < len; i++) {
				...
				if (self.toType (inObj[i]) == "object") {
					...
					self.pathList.push((incomingPathString + suffix));
				}
			}
