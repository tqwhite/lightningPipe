...
var moduleFunction = function(self) {
	...
	self.hash=function(inData){
		if (!this.md5){ this.md5=require('MD5'); }
	...
	self.listProperties = function(inObject, toString) {
		...
		if (type == 'object' || type == 'arguments') {
			...
			for (var i in inObject) {
				...
				outList.push({name:i, string:('item #' + count + ' named \'').grey + i.red + '\' is a '.grey + qtools.toType(element).red + ' value= '.grey+showElement});
				}
				else{
				outList.push({name:i, string:('item #' + count + ' named \'').grey + i.red + '\' is a '.grey + qtools.toType(element).red});
				}
				count++;
			}
