...
var moduleFunction = function(args) {
	...
	var genCreateTableString = function(tableName, propertyNameArray) {
		...
		for (var i = 0, len = propertyNameArray.length; i < len; i++) {
			...
			fieldDefString += '"' + propertyName + '"' + ' TEXT, ';
	...
	var getQuestionMarksString = function(qList) {
		var outString = '';
	...
	var loadOneTable = function(tableName, recList) {
		...
		//		var stmt = db.prepare("INSERT INTO " + tableName + " VALUES (" + getQuestionMarksString(self.fieldNameSequenceLists[tableName]) + ")");
		...
		for (var i = 0, len = recList.length; i < len; i++) {
			...
			for (var j = 0, len2 = fieldNameSequence.length; j < len2; j++) {
				...
				rowString += "'" + element[propertyName] + "', ";
			}
	...
	var generateResults = function(next) {
		...
		for (var i = 0, len = self.export.length; i < len; i++) {
			...
			async.each(taskList, getTableData, function() {
				next()
			});
...
// 			showTables();
// 			showAllCounts();
//			qtools.listProperties(self.outBufferList);
...
//	var db = new sqlite3.Database('/Volumes/qubuntuFileServer/cmerdc/lightningPipe/testResults/sqliteTest');
