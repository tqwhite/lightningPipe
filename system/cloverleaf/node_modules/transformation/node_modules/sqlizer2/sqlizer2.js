...
var moduleFunction = function(args) {
	...
	var validateSourceData=function(){
		
	for (var i=0, len=self.input.length; i<len; i++){
		var element=self.input[i].name;
		if (typeof(self.bufferSet[element])=='undefined'){
			throw "sqlizer2 says, missing input data: "+element;
		}
		
		if (!self.bufferSet[element].hasData()){
			throw "sqlizer2 says, input element: "+element+" is empty";
		}
	...
	var dbIterator = function(queryString, next) {
			global.localEnvironment.log.info({
				source: 'sqlizer2.js/dbIterator',
				type: 'executeSql',
				evidence: {
					query: queryString.substr(0,500)
				}
			});
...
//			showAllCounts();
	...
	if (self.config && self.config.cmdLineSwitches.saveDB) {
		var logDirectory = global.localEnvironment.logFileDirectory,
			fileName = self.fileName?self.fileName:"cloverleafSqliteDb",
