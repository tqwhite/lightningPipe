...
var moduleFunction = function(args) {
	...
	var getAuthParms=function(){
		var authFileName=global.localEnvironment.configFileDirectory+'/dbAuthFiles/'+self.authParmsFile;
		try {
			var fstatResult=fs.statSync(authFileName);
		}
		catch(err){
			throw err;
		}
		...
		try {
			var authParms=fs.readFileSync(authFileName);
		}
		catch(err){
			throw err;
		}

		return JSON.parse(authParms);

	}

	//INITIALIZATION ====================================
	
	self.authParms=getAuthParms();
	
	this.authParms.database=self.databaseName;
	...
	self.takeItAway = function(dataBuffer, fileWriteCallback) {
		...
		var enhancedCallback = function(err, result) {
			var host = self.authParms.host + ":" + self.databaseName + ":" + self.tableName;
