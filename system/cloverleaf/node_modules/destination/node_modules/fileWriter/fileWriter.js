...
var moduleFunction = function(args) {
	...
	if (this.outputSpec.fileFormat=='tabDelimitted'){
		...
		this.flattener=new tmp({config:this.config});
	}
	else{
	...
	this.takeItAway=function(dataBuffer, fileWriteCallback){
		...
		self.dataBuffer+=data;
			...
			var enhancedCallback=function(err, result){
				...
				result=result?result:{targetDataId:filePath}
				fileWriteCallback(err, result);
			}

			qtools.writeSureFile(parentPath+self.fileName, data, {callback:enhancedCallback, append:append});
		return data.length;
	}
