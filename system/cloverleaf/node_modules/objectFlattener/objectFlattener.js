...
var moduleFunction = function(args) {
	...
	var flatten = function(inData) {
		var dottedPathList = qtools.extractDottedPaths(inData[0]),
			outString = '';

		if (self.switches.header) {
	...
	var receiveData = function(err, result) {
		if (err) {
			...
			var data = qtools.getSurePath(result, self.path);
			...
			if (qtools.toType(data) != 'array') {
				...
				self.destination.takeItAway(flatten(data), writeCallback);
			}
		}
	}
