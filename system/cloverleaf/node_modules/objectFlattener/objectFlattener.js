...
// console.log('\n==============  '+ __filename.replace(__dirname,'') +'   (at: if (flatSpecs.fieldList) {)  =========================\n');			
// for (var i = 0, len = workingData.length; i < len; i++) {
// 	var element = workingData[i];
// 
// 	if (element.Student_Base && (element.Student_Base.StudentUniqueIdentifier == '92109060' || element.Student_Base.studentUniqueIdentifier == '92109060')) { 
// 		qtools.dump({
// 			'===== element =====': element
// 		});
// 	}
// }
	...
	var receiveData = function(err, result) {
		...
		if (err || result.Status<0) {
			self.callback({
				message: (result && result.Error) || 'login error or server dead',
				evidence: err
			})
		} else {
...
qtools.dump({'=-=== data =====':data});


				var evidence = {
						dataType: qtools.toType(data),
						badObject: qtools.listProperties(data, 'toString')
					}
				...
				var outString=flatten(data, result.FlatSpecs);
				self.destination.takeItAway(outString, writeCallback);
			}
		}
	}
