	...
	var goGetIt = function() {
		...
		if (lookupControl.type == 'list') {
			...
			switch (lookupControl.finalDataName) {
				...
				case 'schools':
					...
					var query = "select schoolCode as datum from ext.StudentBase group by schoolCode";
						databaseName=self.serverProfile.mapToServerDbNames(district.value);
						self.processQuery(query, databaseName, directoryCallback);
					break;
				case 'segments':
			...
			var dataCallback=function(err, result){
					...
					if (err){
						var errorPackage={message:'=-=== database call error =====', evidence:{error:err, parameters:lookupControl}};
					...
					if (typeof(result)=='undefined'){
						var errorPackage={message:'=-=== database response was undefined =====', evidence:lookupControl};
					...
					if (result.length===0){
						var errorPackage={message:'=-=== no database records were returned =====', evidence:lookupControl};
			...
			segmentName=self.serverProfile.mapToDbTableName(segment.value);
