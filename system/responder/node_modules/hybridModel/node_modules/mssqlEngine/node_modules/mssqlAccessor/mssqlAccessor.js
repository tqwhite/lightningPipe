...
var moduleFunction = function(args) {
	...
	this.processQuery = function(query, databaseName, callback) {
		if (!query){
			callback({error:'query is empty'});
			return;
		}
		if (!databaseName){
			callback({error:'databaseName is empty'});
			return;
		}
		
		self.dataSource.dbParameters.access.database=databaseName;
		console.dir(showAccessParms);
		//log-------
		var showAccessParms=self.dataSource.dbParameters.access;
		showAccessParms.password='****';
		global.localEnvironment.log.debug({lightningPipe:{
	...
	var goGetIt = function() {
		...
		if (lookupControl.type == 'list') {
			...
			var dataCallback=function(err, result){
					if (err){
						var errorPackage={message:'===== database call error =====', evidence:{error:err, parameters:lookupControl.databaseParameters}};
						global.localEnvironment.log.info({message:"ERROR: database call error, check debug log for details"} );
						global.localEnvironment.log.fatal({message:"ERROR: database call error, check debug log for details"} );
					...
					if (typeof(result)=='undefined'){
						var errorPackage={message:'===== database response was undefined =====', evidence:lookupControl.databaseParameters};
						global.localEnvironment.log.info({message:"ERROR: database response was undefined, check debug log for details"} );
						global.localEnvironment.log.fatal({message:"ERROR: database response was undefined, check debug log for details"} );
						global.localEnvironment.log.debug(errorPackage);
						self.emit('gotBadData', errorPackage);
						return;
					}
					...
					if (result.length===0){
						var errorPackage={message:'===== no database records were returned =====', evidence:lookupControl.databaseParameters};
						global.localEnvironment.log.info({message:"ERROR: no database records were returned, check debug log for details"} );
						global.localEnvironment.log.fatal({message:"ERROR: no database records were returned, check debug log for details"} );
						global.localEnvironment.log.debug(errorPackage);
						self.emit('gotBadData', errorPackage);
						return;
					}
			...
			lookupControl.databaseParameters={databaseName:databaseName, query:query};
