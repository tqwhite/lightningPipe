...
var processForSchedule = function(sqlizer) {
	...
	sqlizer.mapColumns({
			...
			"Section_Student.StudentUniqueIdentifier": function(record) {
				var value=record.Section_Student.LocalStudentNumber.substr(4)
...
var processForGuardians=function(sqlizer){
		...
		sqlizer.mapColumns({
			...
			"Address_Contact.StudentUniqueIdentifier": function(record) {
				var value=record.Address_Contact.LocalStudentNumber.substr(4)
		...
		sqlizer.select(function(record){
			...
			if (LocalStudentNumber.toString().length>4){
			return true;
			}
			//else
			return false;
