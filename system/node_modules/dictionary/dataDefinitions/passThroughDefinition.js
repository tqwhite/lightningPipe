...
var processForSchedule = function(sqlizer) {
	...
	sqlizer.mapColumns({
		"Section_Student.Address_Contact.StudentUniqueIdentifier": function(record) {
			var value = 'StudentUniqueIdentifierGoesHere'
...
var processForGuardians=function(sqlizer){
		...
		sqlizer.mapColumns({
			"Address_Contact.Relationship": function(record) {
				var value = 'RelationshipGoesHere'
			...
			"Address_Contact.Guardseq": function(record) {
				var value = 'GuardseqGoesHere'

				return value;
			},
			
			"Address_Contact.LegalGuard": function(record) {
				var value = 'LegalGuardGoesHere'
		...
		sqlizer.select(function(record){
			...
			if (LocalStudentNumber.toString().substring(0,4)!=''){
			return true;
			}
			//else
			return false;
...
module.exports = {
	...
	"Schedule": {
		...
		"assembler": {
			"expressbook": {
				name: 'pseudoSql', //note: this refers to an assembler in the dataAccess module package. Could refer to a fully qualified path to an assembler elsewhere.
				params: {finalProcess:processForSchedule}
			}
		}
	},
	...
	"Guardian": {
		...
		"assembler": {
			"expressbook": {
				name: 'pseudoSql', //note: this refers to an assembler in the dataAccess module package. Could refer to a fully qualified path to an assembler elsewhere.
				params: {finalProcess:processForGuardians}
			}
		}
	},
