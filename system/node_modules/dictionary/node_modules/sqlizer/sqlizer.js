...
var moduleFunction = function(args) {
	...
	this.setBaseTable = function(tableName) {
		...
		if (global.localEnvironment.testServer && !this.tables[tableName]) {
			qtools.message("Invalid tableName: " + tableName + " does not exist");
		}
		if (global.localEnvironment.testServer) {
			qtools.message("SQL Base Table " + tableName + " has " + this.tables[tableName].length + " records");
		}
	...
	this.innerJoin=function(linkColumn, targetTableName, targetLinkColumn){
		...
		if (!self.indexes[targetTableName] || !self.indexes[targetTableName][targetLinkColumn]){ self.index(targetTableName, targetLinkColumn);}
	...
	this.getResult = function() {
		if (global.localEnvironment.testServer) {
			qtools.message("SQL Result: " + this.workingTable.length + " records");
		}
