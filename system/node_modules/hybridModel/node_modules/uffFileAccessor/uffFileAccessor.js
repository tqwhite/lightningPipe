	...
	var digestQuery = function(query, path, newQuery, status) {
		...
		if (qtools.isEmpty(query)) {
			...
			if (current.value=='list'){
				...
				fileName=fileName?fileName:current.value;
				var clientFileName=self.clientProfile.dataSource.schemaSourceNameMapping[current.value];
qtools.dump({'\n\n===== self.clientProfile.dataSource.schemaSourceNameMapping =====\n':self.clientProfile.dataSource.schemaSourceNameMapping});
				fileName=clientFileName?clientFileName:fileName;
	...
	this.goGetIt = function() {
		...
		var path = basePath + lookupControl.path[0];
		
		/*
			to make multiple file lookups...
			1) make it so that on('got***Data') knows that it's collecting a list of files
			2) move convert() to gotRawData
			3) make digestQuery consult converter to find out what files are needed
			4) make the definition tell digestQuery multiple files
				make it convert and combine the simply
		*/
