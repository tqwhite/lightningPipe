...
{"query":"create table Users as select distinct 'v2.13MAPPED user parents primary and secondary' as version, j.personId as personId, j.userName as userName, 'Te$tUs3r' as password, j.contactMiddleName as firstName, j.contactFullName as lastName, 'tmp@tmp.tmp' as email, '' as city, '' as street, '' as phone1, '' as phone2, '' as grade, '' as gradYear, j.schoolCode as schoolCode, 'PARENT' as department, 'US' as country, '' as description from Address_Contact as j where trim(personId) <> '' and trim(userName) <> '' and trim(password) <> '' and trim(firstName) <> '' and trim(lastName) <> '' and trim(email) <> '' "},
...
"summaryFunction":"function(item, inx, entire) {  /* v2.10 courseGroup Summary Function */  var termString = '',   alreadyIn=[],   outObj=item[0];     for (var i = 0, len = item.length; i < len; i++) {   var element = item[i];      if (element.role.toLowerCase()!='editingTeacher'.toLowerCase()){    continue;   }      if (alreadyIn.indexOf(element.classGroup)<0){   termString += element.classGroup + ',';   alreadyIn.push(element.classGroup);   }  }  termString = termString.replace(/,$/, '');   outObj.classGroup=termString;  return outObj; }"
    ...
    },   
    {
		...
		"process":[
{"query":"update Enrollments /* v2.11 update courseGroups secondRoundSql*/ set classGroup=(select classGroup from classGroupNames where Enrollments.courseId=classGroupNames.courseId)"},
{"query":"create table /* v2.11 distinct enrollments secondRound */ EnrollmentsDistinct as select distinct * from Enrollments"}
    ...
    "output": {
        "type": "mysql",
        "context": {
            "parentPath": "/Users/tqwhite/testLinkpoint/testDataDest/TCUJsonTest/030/",
            "authParms":{
				"host":"MySQL64-ELEARNING",
				"port":"3306",
				"user":"root",
				"password":"eag!312",
				"database":"zzTqFromMac_TCU030"
			}
        },
